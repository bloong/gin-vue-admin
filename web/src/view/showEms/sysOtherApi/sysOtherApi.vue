...
<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" :label="t('showEms.OtherApi.Test')" prop="test" width="120" />
        <el-table-column align="left" :label="t('general.operations')" fixed="right" min-width="240">
          <template #default="scope">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateOtherApiFunc(scope.row)">{{t('general.change')}}</el-button>
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow">
            <el-descriptions :column="1" border>
                    <el-descriptions-item :label="t('showEms.OtherApi.Test')">
                        {{ detailFrom.test }}
...
const getTableData = async() => {
  const table = await getOtherApiList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm(t('general.deleteConfirm'), t('general.hint'), {
    ...
    type: 'warning'
      ...
      const res = await deleteOtherApiByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
...
// 更新行
const updateOtherApiFunc = async(row) => {
    const res = await findOtherApi({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteOtherApiFunc = async (row) => {
    const res = await deleteOtherApi({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: t('general.deleteSuccess')
