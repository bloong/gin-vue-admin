...
<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" :label="t('showEms.DITable.InterfaceName')" prop="interfaceName" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.interfaceName,DIOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" :label="t('showEms.DITable.RealStatus')" prop="realStatus" width="120">
            <template #default="scope">{{ formatBoolean(scope.row.realStatus) }}</template>
        </el-table-column>
        <el-table-column align="left" :label="t('showEms.DITable.UseFor')" prop="useFor" width="120" />
        ...
        <el-table-column align="left" :label="t('general.operations')" fixed="right" min-width="240">
          <template #default="scope">
            ...
            <el-button v-auth="btnAuth.edit" type="primary" link icon="edit" class="table-button" @click="updateDITableFunc(scope.row)">{{t('general.change')}}</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item :label="t('showEms.DITable.RealStatus')"  prop="realStatus" >
              <el-switch v-model="formData.realStatus" active-color="#13ce66" inactive-color="#ff4949" :active-text="t('general.yes')" :inactive-text="t('general.no')" clearable ></el-switch>
            </el-form-item>
...
const onSubmit = () => {
  elSearchFormRef.value?.validate(async(valid) => {
    ...
    page.value = 1
    pageSize.value = 10
    if (searchInfo.value.realStatus === ""){
        searchInfo.value.realStatus=null
    }
    getTableData()
  })
}
...
const getTableData = async() => {
  const table = await getDITableList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm(t('general.deleteConfirm'), t('general.hint'), {
    ...
    type: 'warning'
      ...
      const res = await deleteDITableByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
...
// 更新行
const updateDITableFunc = async(row) => {
    const res = await findDITable({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteDITableFunc = async (row) => {
    const res = await deleteDITable({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: t('general.deleteSuccess')
